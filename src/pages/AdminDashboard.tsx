...
export default function AdminDashboard() {
  const lowStockItems = MOCK_PRODUCTS.filter(p => p.stock < 10);
  return (
              ...
              {lowStockItems.map(item => (
                <div key={item.id} className="flex items-center justify-between p-3 rounded-lg border border-slate-100 bg-slate-50/50">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500">Stock: <span className="text-red-500 font-bold">{item.stock} {item.unit}s</span></p>
                  </div>
