...
import { Receipt, Calendar, CreditCard, ShoppingCart, TrendingUp, RefreshCw, Loader2 } from 'lucide-react';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="py-8 md:py-10 lg:py-12 space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <KPICard title="Total Revenue" value={`₵${totalRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`} icon={TrendingUp} color="emerald" />
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
          ...
          <ScrollArea className="h-[600px]">
            {isLoading ? (
              ...
              <div className="p-20 text-center text-muted-foreground italic">No transactions recorded yet.</div>
            ) : (
                      ...
                      <div className="flex items-center gap-8">
                        ...
                        <div className="flex flex-col items-end gap-1">
                           <Badge className="bg-blue-50 text-blue-700 hover:bg-blue-50 border-none gap-1 dark:bg-blue-900/20 dark:text-blue-400">
                             <CreditCard className="w-3 h-3" /> {sale.paymentMethod?.toUpperCase() || 'CASH'}
                           </Badge>
                        </div>
                      </div>
                    </div>
  ...
  return (
    <Card className="shadow-sm border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
      <CardContent className="pt-6">
        ...
        <div className="mt-4">
          <h3 className="text-sm font-medium text-muted-foreground uppercase tracking-tight">{title}</h3>
