    ...
    <div className="p-6 space-y-6">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-500 animate-pulse font-medium">Syncing product catalog...</p>
        </div>
      ) : (
          ...
          {filtered.map((product) => {
            ...
            return (
              <Card
                key={product.id}
                className={cn(
                  "group cursor-pointer hover:border-emerald-500 transition-all active:scale-95 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900",
      ...
      {!isLoading && filtered.length === 0 && (
        <div className="text-center py-20">
          <p className="text-slate-400 italic">No products found matching "{search}"</p>
        </div>
      )}
    </div>
  );
